...
const DropdownMenu = () => {
    ...
    return (
      <div>
        <button onClick={toggleMenu} data-collapse-toggle="navbar-default" type="button" className="inline-flex items-center p-2 mr-4 w-10 h-10 justify-center text-sm text-slate-500 rounded-lg md:hidden hover:bg-slate-100 focus:outline-none dark:text-slate-400 dark:hover:bg-slate-700" aria-controls="navbar-default" aria-expanded="false">
        ...
        <div className={`fixed top-0 left-0 bg-[#1E1F26] text-white w-96 z-50 h-full transition-transform duration-500 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
            ...
            <div className="px-6" id="links">
                ...
                <a
                    key={path.name}
                    href={path.link}
                    className={`py-4 text-3xl block font-bold transition-transform duration-500 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
